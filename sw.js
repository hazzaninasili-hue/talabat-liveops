const CACHE_VERSION = 'talabat-v1781107275';
...
self.addEventListener('fetch', e => {
  ...
  if (e.request.mode === 'navigate' || url.pathname.endsWith('index.html') || url.pathname === '/talabat-liveops/' || url.pathname === '/talabat-liveops') {
    e.respondWith(fetch(e.request).then(res => { caches.open(CACHE_NAME).then(c => c.put(e.request, res.clone())); return res; }).catch(() => caches.match('./index.html')));
    return;
  }
